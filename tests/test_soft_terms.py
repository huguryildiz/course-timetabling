...
def test_norm_obj_weights_and_normalization():
    base = {"idle": 10, "maxrun": 5, "instr_days": 20, "room_stable": 4, "free_day": 2, "conf": 0}
    cfg = Config(w_idle=15, w_maxrun=10, w_instr_days=10, w_room_stable=10, w_free_day=10)
    # at base, every term/base == 1 -> objective == sum of weights
    assert _norm_obj(base, base, cfg) == 15 + 10 + 10 + 10 + 10
    halved = dict(base, idle=5)               # idle halved -> drops 7.5
    assert abs(_norm_obj(halved, base, cfg) - (15 * 0.5 + 10 + 10 + 10 + 10)) < 1e-9
